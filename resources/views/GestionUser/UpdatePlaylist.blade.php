...
<body class="bg-gray-300"  style="background-image: url(./images/addsong.jpg); object-fit:cover;">
    ...
    <section class="bg-blue-300 w-[60%]  m-auto rounded-sm  p-4 border border-gray-500 mt-36 " >
        <form action="/UpdatePlaylist/{{$playlist->id}}" method="post" enctype="multipart/form-data" class="flex  justify-around items-center m-auto  h-96 ">
            
            @csrf
            @method('PUT')
            <div class=" rounded p-6 w-80 h-44 bg-white">
                <input type="file" id="file-upload" class="hidden" name="image" value="{{old('image')}}">
            ...
            <div class="flex flex-col justify-between h-28"> 
                ...
                <input type="text" name="name" class="rounded w-72  py-2 " placeholder="Name of Playlist"  value="{{$playlist->title}}" >
                @error('name')
                <p class="text-red-500 text-xs mt-1">{{$message}}</p>
            @enderror
            </div>
