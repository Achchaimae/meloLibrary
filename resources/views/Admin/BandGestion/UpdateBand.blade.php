...
<body class="bg-gray-700 "  style="background-image: url(./images/background.jpg); object-fit:cover;">
    ...
    <div class="mx-auto max-w-screen-xl px-4 pb-2 sm:px-6 lg:px-8    ">
        <div class="mx-auto max-w-lg">
            ...
            <form action="/UpdateBand/{{$band->id}}" method="post"  enctype="multipart/form-data" class="mt-6 mb-0 space-y-4 rounded-lg p-4 shadow-lg sm:p-6 lg:p-8 bg-white bg-opacity-90">
                 @csrf
                @method('PUT')
                <label for="image" >insert an image for the band</label>
                <input type="file" name="image"  value="{{old('image')}}" class="w-full rounded-lg border-gray-200 p-4 text-sm shadow-sm ltr:pr-12 rtl:pl-12">
                 @error('image')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror 
                <label for="name">name</label>
                <input type="text" name="name" id="name"  value="{{old('name')}}" class="w-full rounded-lg border-gray-200 p-4 text-sm shadow-sm ltr:pr-12 rtl:pl-12">
                 @error('name')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror 
                ...
                <input type="date" name="release_date" value="{{old('release_date')}}" class="w-full rounded-lg border-gray-200 p-4 text-sm shadow-sm ltr:pr-12 rtl:pl-12">
                 @error('release_date')
                    <p class="text-red-500 text-xs mt-1">{{$message}}</p>
                @enderror 
